...
const modalWindow = h('div#modalWindow.modal.fade',
  ...
  h('div.modal-dialog',
    {
      role: 'document',
      class: { 'modal-lg': G('features.modal-large') }
    },
    h('div.modal-content',
      h('div.modal-header',
        h('h5#modalWindowLabel.modal-title'),
        h('button.close', h('span', h.safe('&times;')))
      ),
      h('div.modal-body'),
      h('div.modal-footer')
    )
...
module.exports = h('html',
  h('head',
    ...
    h('link', { rel: 'stylesheet', href: publicPath('snooty.css') }),
